...
> it listed out the entire vector omitting the third and first items in the list. 
...
> In the first instance (true and false) we are assigning true or false values to each item in the list. So it is run, only the true items are coming back, the false omitted. In the second instance we are asking for only items in the list greater than the number 7 to returned. 
...
I *think* that what happened is that you made a variable, y, then assigned the first four items in the x variable character names, a-d. 
...
> The second example builds a matrix including the first three rows (1-3) and then the comma indicates that you want all the columns for those rows. Essentially building a 3x14 matrix. 
...
> I wanted to find the population of Midwestern states from when the term came into usage (the 1890s) to the present day. 
...
>This pulls up only the city names for the rows 100-110. 
...
over_50 <- BostonWomenVoters[BostonWomenVoters$Age < 50,]
under_50 <- BostonWomenVoters[BostonWomenVoters$Age >50,]
...
lives_on_main <- BostonWomenVoters[grep("Main", BostonWomenVoters$Street.of.Residence.on.April.1),]
...
# this gets us the last 5 in the list, sorted by age.
...
#however, the last five do not have ages, so the last five with ages:
