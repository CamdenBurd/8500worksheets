...
```{r}
paste("Hello", "my friends, let's unionize")
```

(@) Can you explain what happened in 2-3 sentences? 

> Sure, paste add two seperate strings or characters together in this case, it was left leaning encouraging friends to come together in order to claim greater autonomy in the workplace. 
...
> 5 equals 10
3 is less than 4
five does not equal 10
five does not equal 5
three is equal to four or three
three does not equal four and three does not equal 5
...
> It took each number in the vector and multiplied each by two, listing them in their corresponding order. 
...
> You selected the third item in the list and then multiplied it by 2. 

(@) You try, create a list of five items and store it in a descriptive variable. 
```{r}
numbers.list <- c(1, 3, 5, 7, 9)
```
...
> per the guide, "takes a single numeric argument x and returns a numeric vector containing the largest integers not greater than the corresponding elements of x."
...
> column z is the sum of x and y in the corresponding row. 
...
> gayguides is identifying the dataset, the $ is directing us(?) to the specific column title. Then the bracketed 500 is calling the 500th iteam in that column. 
...
> Its a data set. 10,702 rows and 34 columns or variables. Its a mix of character and integer variables. 
