...
gg_regions <- left_join(gayguides, regions, by = c("state"="State.Code"))

```

(@) Explain what you did above. What variable did you join by and why? What results?

# I made magic happen. Nah, really, I did a left joing and connected the state column in GG with the state.code column in regions. But this is the first I am learning of the "by" action and that's dope as shit. 
...
gg_growth_region <- gg_regions %>%
  filter(Year > "1970", Year < "1980") %>%
  select(Region, Year) %>%
  group_by(Region, Year) %>%
  summarise(count =n())
 
 gg_growth_state <- gg_regions %>%
  filter(Year > "1970", Year < "1980") %>%
  select(state, Year) %>%
  group_by(state, Year) %>%
  summarise(count =n())
...
> it skipped over city and type of worker, then took the rest and collapsed into "names_to" then added a count column. 
...
rec.spaces_transformed <- rec.spaces %>%
...
> several versions of housekeepers. variations of widows. 
...
> of those collected, cashier is the lowest age. 
...
```{r}
# several variations of housewife
BWV.occupations_1 <- BostonWomenVoters %>%
    mutate(
      Occupation_clean_more = fct_recode(Occupation,
        "Housewife" = "housewife",
        "Housewife" = "Married, Housewife",
        "Housewife" = "Housewife ",
        "Housewife" = "Married",
        "Housewife" = "housewife, married",
        "Housewife" = "house-wife",
        "Housewife" = "House-wife"
      )
    ) %>%
    count(Occupation_clean_more) %>%
    arrange(desc(n))
...
```{r}
test_2 <- BostonWomenVoters %>%
  mutate(occupation_categories = fct_collapse(Occupation,
  "Domestic" = c("Housewife", "Housekeeper", "At Home", "Housework", "housewife", "Housewife", "At home"),
  "Medical" = c("Nurse", "nurse", "Physician"),
  "Education" = c("Teacher", "School teacher", "Student"),
  "Textiles" = c("dressmaker", "Tailoress", "Laundress")
  )
  ) %>% 
  count(occupation_categories) %>%
  arrange(desc(n))
...
# Ehh, I just started group shit based on profession. 
...
class_status <- BostonWomenVoters %>%
  mutate(
    occupation_historical = fct_collapse(Occupation,
  "Working Class" = c("Factory Worker", "Factory Work", "Factory", "factory hand"),
  "Professional Managerial Class" = c("Editor", "Teacher", "teacher", "Teacher ", "Teacher  "),
  "Elite" = c("Housewife", "housewife")
  ),
  occupation_historical = fct_relevel(occupation_historical, "Elite", "Professional Managerial Class", "Working Class") 
  
  ) %>%
  pull(occupation_historical) %>%
  levels()


```

> Explanation: Ehh, I tried to pull a few careers based on class status. 
...
```{r}
gayguides_date <- gayguides %>%
  mutate(full_date = paste("12", "1", gayguides$Year, sep = "-"))

gayguides_date_1 <- gayguides_date %>%
  mutate(new_date = mdy(full_date))
...
gg_decades <- paste(floor(year(gayguides_date_1$new_date)/10)*10)
...
BWV_birthyear <- BostonWomenVoters %>%
  mutate (birth_date = (paste("7", "1", (1920 - BostonWomenVoters$Age), sep = "-"))) %>%
  mutate(real_birthday = mdy(BWV_birthyear$birth_date))
...
dec_1960s <- interval(ymd("1960-01-01"), ymd("1969-12-31"))
dec_1970s <- interval(ymd("1970-01-01"), ymd("1979-12-31"))
dec_1980s <- interval(ymd("1980-01-01"), ymd("1989-12-31"))

for(i in 1:length(gayguides_date_1$new_date))
  if(gayguides_date_1$new_date[i] %within% dec_1960s){
  print("This is the 1960s")
} else if (gayguides_date_1$new_date[i] %within% dec_1970s) {
   print("This is the 1970s")
} else if (gayguides_date_1$new_date[i] %within% dec_1980s) {
   print("This is the 1980s")
}
...
make_a_date <- function(any_year){
  ymd(paste0(any_year, "-01-01"))
}
make_a_date(2019)
...
> Many dates are only by years. Or other dates will be inconsistent with other forms of data. Not all sources may have consistent naming mechanisms or accurate record keeping. Lubridate can create some seamless options if you have a better sense of months or even years. 
...
```{r}
Looking at the total number of colleges where judges hailed from, is there preference among political parties? 
...
```{r}
#Is there any preference about the party that selects the judges from schools? 

judges %>%
  select(Party.of.Appointing.President..1.,School..1.) %>%
  group_by(Party.of.Appointing.President..1., School..1.) %>%
  summarise(count = n()) %>%
  arrange(desc(count))
...
```{r}
Is there a preference for type of locale in relation to year? 
```
...
gg_cleanedup <- function(year, state) {
  for (i in 1:nrow(gayguides)){
  if (gayguides$Year[i] == year & gayguides$state[i] == state) {
    print(paste(gayguides$title[i], gayguides$type[i], gayguides$state[i], gayguides$Year[i], sep = ", "))}
  }
}

gg_cleanedup(1985, "NY")
...
newbwv <- BostonWomenVoters %>% 
  drop_na(Ward, Age) %>%
  mutate(occupation_groups = fct_collapse(Occupation, 
  "Education" = c("Teacher", "School teacher", "Stenographer"),
  "Retail" = c("clerk", "Clerk", "Saleslady", "Book keeper", "Book-keeper", "Bank clerk"),
  "Domestics" = c("Housewife", "at home", "At home"),
  "Textiles" = c("Drapery sewer", "Weaver"),)) %>%
  group_by(Ward, occupation_groups) %>%
  mutate(Age, average_age = mean(Age)) %>%
  mutate(Age, min_age = min(Age)) %>%
  mutate(Age, max_age = max(Age)) %>%
  select(Ward, occupation_groups, average_age, min_age, max_age) %>% 
  arrange(Ward)
...
> I am not sure to be honest, ha. Workers I grouped in retail schew younger than those I categorized as Domestic. Which makes sense as many of those contined housewifes. 
