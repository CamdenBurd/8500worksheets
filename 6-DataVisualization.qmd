...
> ## its got information on countries, continents, years, population, life expectancy,and per capita. In short, its census-adajacent data.  
...
```{r}
test1 <- ggplot(data = gapminder, mapping = aes(x = lifeExp, y = population)) + geom_point()
```
...
> Bins collect the data. By chaning bins, it will collect data accordinly. More bins, more granualar data collected in each bin. Less bins, collects more of the data.  
...
gg_se_states <- gayguides %>% group_by(state, Year) %>% filter(Year == 1980) %>% summarise(count = n()) %>% filter(state %in% c("SC", "GA", "NC")) 


ggplot(gg_se_states, aes(x = state, y = count)) + geom_col()
```

(@) Using the `gayguides` data, plot the number of religious institutions in each state.
```{r}
unique_type <- unique(gayguides$type)

gg_religion <- gayguides %>% filter(type == "Religious Institution") %>% group_by(state) %>% summarise(count = n())

ggplot(gg_religion, aes(x = state, y = count)) + geom_col()
...
< Well, I I've been waiting to use my own data for a question about parks and types of parks introduced into the federal system over time. 

(@) Now write the code to answer that question and use comments (`#`) to explain your thinking as you go.
```{r}

ggplot(park_category, aes(x = decade, y = count, color = designation)) + geom_col()
...
```{r}
gg_religion <- gayguides %>% filter(type == "Religious Institution") %>% group_by(Year) %>% summarise(count = n())

ggplot(gg_religion, aes(x = Year, y = count)) + geom_line()
...
```{r}
gg_danger <- gayguides %>% select(amenityfeatures, description, streetaddress, Year) %>% filter(grepl("HOT|AYOR", amenityfeatures)) %>% group_by(Year) %>% summarise(count = n())

ggplot(gg_danger, aes(x = Year, y = count)) + geom_line()
```
...
```{r}
rec_city_exp <- recreational.data %>% select(city, total_expenditures)

ggplot(recreational.data, aes(x = city, y = total_expenditures)) + geom_col() + facet_wrap(~city)
```

(@) Using the gayguides data can you select 5 types of locations and chart the number of each type between 1965 and 1980? 
```{r}

gg_types <- gayguides %>% filter(type == "Cruising Area"| type == "Book Store" | type == "Restaurant" | type == "Bars/Clubs" | type == "Theatre") %>% filter(Year >= 1965 | Year <= 1980) %>% group_by(type, Year) %>% summarise(count = n())

ggplot(gg_types, aes(x = Year, y = count)) + geom_line() + facet_wrap(~type)
...
rec_worker_pivot <- rec.workers %>% pivot_longer(!city:type_of_worker, names_to = "year", values_to = "count")

rec_worker_data <- rec_worker_pivot %>% mutate(worker = fct_collapse(type_of_worker, "paid" = c("male paid", "female paid"), "volunteer" = c("men volunteer", "women volunteer"))) %>% select(worker, year, count) %>% group_by(worker, year) %>% summarise(count = mean(count, na.rm = TRUE)) %>% mutate(year = str_remove(year, "^X")) %>% mutate(year = as.numeric(year))


ggplot(rec_worker_data, aes(x = year, y = count)) + geom_line() + facet_wrap(~worker) 


#### use fact_collapse to make the new factors for volunteer and nonvulunterr labor
##### need to make the year a number column by removing the x in the columbe. use mutate using gsub 
...
gg_mountainwest_time <- gayguides %>% select(state, Year) %>% filter(state %in% c("UT", "NV", "CO", "AZ", "MT")) %>% group_by(state, Year) %>% summarise(count = n())

ggplot(gg_mountainwest_time, aes(x = Year, y = count, color = state)) + geom_line()

```

(@) Plot 1 explanation: 
> I wanted to see the number of locations in Mountain West states over time. 

(@) Plot 2
```{r}

gg_midwest <- gayguides %>% select(state, Year) %>% filter(state %in% c("MI", "IN", "IL", "OH", "WI", "MN", "IA", "NE")) %>% group_by(state, Year) %>% summarise(count = n())

ggplot(gg_midwest, aes(x = Year, y = count, color = state)) + geom_line()


```

(@) Plot 2 explanation: 
> Same as the first, but for the Midwest. 
