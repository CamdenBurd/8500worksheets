...
rec_worker_data <- rec_worker_pivot %>% select(type_of_worker, year, count) %>% group_by(type_of_worker, year, count) %>% summarise(mean = mean(count))

ggplot(rec_worker_data, aes(x = year, y = count)) + geom_col() + facet_wrap(~type_of_worker) ty 
