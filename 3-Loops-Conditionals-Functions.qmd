...
```{r}
hist_100 <- 35
if (hist_100 > 30) print("enrollment has been met")
```
...
if (length(presidents) > 5) {
  print ("this is the first five presidents")
} else if (length(presidents) < 5) {
  print ("there are not five presidents here")
...
gayguides_years <- gayguides[order(gayguides$Year),]


last_gg_item <- tail(gayguides_years, n=1)
print(last_gg_item)

if(last_gg_item$Year > 1990){
  print("There is a year beyond 1990")
} else {
   print("There is no entry beyond 1990")
}
...
> It took too long but I got there. Order the Gayguides database by year, ascending. Then call the last item. Then have an if statement that checks in the value of the last "year" is greater than 1990. 
...
>  length is simply running through the list of characters that you have outlined in the variable. 
...
```{r}
spring26_courseload <- c("hist 8540", "hist 8550", "coding is fun, i'm told")

for(c in 1:length(spring26_courseload)) {
  print(paste("I am teaching or enrolled in ", spring26_courseload[c]))
}
```
...
```{r}
gg_first5 <- head(gayguides, n = 5)
for(i in 1:length(gg_first5$title)) {
  print(gg_first5$title[i])
}
...
```{r}
ggbars <- gayguides[grepl("Bar", gayguides$title),]


for (i in 1:length(ggbars$title)) {
  print(ggbars$title[i])
}
...
The skip command ensured that numbers 1 and 2 would not be printed. The break command ended the loop at 5. 
...
```{r}
historyStudents("Camden", "HIST 8540")
```
...
greet <- function(studentname){
  statement <- paste("Hello ", studentname, "!", sep = "")
  statement
}
...
ch_function <- function(text) {
  character_number <- nchar(text)
  print(paste("There are ", character_number, " characters in that string.", sep = ""))
  
}
ch_function("this is a sentence")
...
> started with the print/paste function and worked backwards. In between those moments, I threw things against the wall. I went through like 16 computers. Put in for a reimbursement with college but they rejected me claiming that computers were not "mission criticial" to the DH program. 
...
> Because you'd have to the new variable, George Washington. 
...
gg.state <- function (d.state, d.year) {
  data(gayguides)
...
bb.occupation1 <- function (occupation) {
  data(BostonWomenVoters)
  bb.result <- data.frame()


  for (i in 1:nrow(BostonWomenVoters)) {
    if (BostonWomenVoters$Occupation[i] == occupation) {
      bb.result <- rbind(bb.result, BostonWomenVoters[i, ])
    } 
  }
...
state_count <- 0
  ...
  for (i in 1:length(gayguides$state)){
   ...
   print(paste("There are ", state_count, "in the ", state))
}
count_by_state ("NY")
...
> They all equally work well at making me very, very angry. No, you write a function (a baby program) that holds loops and conditionals to quickly process your data and provide new outputs. 
