...
undergroundRRgeocode <- undergroundRR %>% geocode(city = City, state = State, method='osm', lat = latitude, long = longitude)

head(undergroundRRgeocode)
```
...
boston_fulladdres <- BostonWomenVoters %>% mutate(fulladdress = paste(Street.Number.on.April.1, Street.of.Residence.on.April.1, sep = " ")) %>% mutate(city = "Boston") %>% mutate(state = "MA")

boston_geocode2 <- boston_fulladdres %>% slice(5:75) %>% geocode( address = fulladdress, lat = latitude, long = longitude) %>% drop_na(longitude, latitude)
...
>  It is a map with a vareity of places. Thus a map of America. 
...
```{r}
ggplot() + 
  geom_map(data = usa, map = usa, aes(long, lat, map_id=region), fill = "lightgray", color = "black") + coord_fixed(1.3) +
...
```{r}
gayguides_nahawaii <- gayguides %>% filter(state != "HI") %>% filter(Year == 1965)

ggplot() + 
  geom_map(data = usa, map = usa, aes(long, lat, map_id=region), fill = "lightgray", color = "black") + coord_fixed(1.3) +
  geom_point(data = gayguides_nahawaii, mapping = aes(x = lon, y = lat), color = "red")

```

(@) Can you map just locations in South Carolina (on a basemap of SC)? 
```{r}
sc_basemap <- usa %>% filter(region == "south carolina")

gayguides_sc <- gayguides %>% filter(state == "SC")

ggplot() + 
  geom_map(data = sc_basemap, map = sc_basemap, aes(long, lat, map_id=region), fill = "lightgray", color = "black") + coord_fixed(1.3) +
  geom_point(data = gayguides_sc, mapping = aes(x = lon, y = lat), color = "red")
...
ma_basemap <- usa %>% filter(region == "massachusetts")

ggplot() + 
  geom_map(data = ma_basemap, map = ma_basemap, aes(long, lat, map_id=region), fill = "lightgray", color = "black") + coord_fixed(1.3) +
  geom_point(data = boston_geocode2, mapping = aes(x = longitude, y = latitude), color = "red")
