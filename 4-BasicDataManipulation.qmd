...
> I had to make a new variable so that those selected columbns would permanently stores in "locations"
...
```{r}
gg_sanslocations <- gayguides %>% select(-city, -state)
```
...
```{r}
Greenville_SC <- gayguides %>% filter(city == "Greenville", state == "SC")
```

(@) How about every location between 1975 and 1980? 

```{r}
late_1970s <- gayguides %>% filter(Year > 1975 & Year < 1980)
```
...
```{r}
greenville_late70s <- gayguides %>% filter(city == "Greenville", state == "SC", Year > 1975 & Year < 1980)
```

(@) Can you find all locations in 1975 except for New York and San Francisco? 
```{r}
locations_1975_sansNYandSF <- gayguides %>% filter(city != "New York City", city != "San Francisco", Year == 1975)
```
...
```{r}
g_l_things <- gayguides %>% filter(grepl("(G)|(L)", amenityfeatures))
```
...
```{r}
BostonWomenVoters %>% mutate(birth_year = 1920 - Age)
...
```{r}
gayguides %>% mutate(city_capitalize = str_to_title(city))
...
```{r}
gg_dancing <- gayguides %>% mutate(has_dancing = grepl("(D)", amenityfeatures))
```

(@) In the Boston Women Voters data, occupation names may have extra whitespace. Use `str_trim()` to clean the occupation column.
```{r}
boston_clean_occupation <- BostonWomenVoters %>% mutate(clean_occupation = str_trim(Occupation))
```
...
sum(is.na(BostonWomenVoters))
```

(@) Why might ward information be missing for some voters? What does this tell us about the historical record?

> That its not always known which ward a person lived in based on historical record. 
...
```{r}
boston_tf <- BostonWomenVoters %>% mutate(ward_missing = is.na(Ward))
...
levels(gg_state_factor$state_factor)
...
```{r}
gg_typefactor<- gayguides %>% mutate(type_factor = as.factor(type))
levels(gg_typefactor$type_factor)

```

(@) In the Boston Women Voters data, convert Occupation to a factor. What are the levels?
```{r}
boston_occupation_factor <- BostonWomenVoters %>% mutate(occupation_factor = as.factor(Occupation))

levels(boston_occupation_factor$occupation_factor)
