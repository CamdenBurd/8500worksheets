...
```{r}
ac.top.terms_v2 <- ac.topics %>%
  arrange(desc(beta)) %>% 
  group_by(topic) %>% filter(topic == 1) %>% slice_head(n = 10)
...
Topic 1:(Not really, they are very similar) Water Infrastucture
Topic 2: Also Water infrascture 
Topic 3: New York Water
Topic 4: Public Water
Topic 5: New York Public Water
...
> I chose Information Bulletin--not sure why. Just cause. I am mostly going to use it to run through programs through seeing whats in the the thing.  
...
> I'll choose to track some topics over time. To see what sort of discourse changes in corpus in as the post-WW2 era progresses. 
