...
sotu_sentences <- sotu_whole %>%
  unnest_tokens(output = sentence, text, token = "sentences")
```
...
> We are taking the loaded meta data and creatign a dataframe from it "meta". We then are creating a directory to house the sotu files and the file paths for those files. Readtext then pulls text from the file paths variable. The new data is organized by document id and the text. You then take the metadata, arrange by president, then bind colums (though I am not sure by what?) TidySotu is might by tokening the text of the speeches, by wods, then removing the stopwords. 
...
> The STOU used to be a written document, submitted to Congress. Whereas, in the modern age, the STOU is a speech.  

(@) Filter the dataset to address this discrepancy and the recreate these statistics: 
```{r}


sotu.length.written <- tidy_sotu %>%
...
## not sure if this is what you are looking for, but we now have an average of length based on written vs spoken STOU as separate data frames. 
```
...
> Johnson's highest tf_idf score came through was Vitenam. Which is both interesting because it was sort of a defining aspect of his presidency. But it was especially poignant in 1966? 
...
mind_body_full <- left_join(mb, metadata, by = c("doc_id" = "Filename"))
```
...
mb_texts <- mind_body_full %>%
  ...
  # take the joined table and create a new dataframe where the words are tokenized and stopwods are removed. 


  # also went back and added a few custom stop words because i was getting too much workout terminology. I would still edit this. 

  mb_custom_stops <- tibble(word = c("quarte", "tierce", "parry", "meas"))
  updated_custom_stops <- bind_rows(stop_words, mb_custom_stops)

# for my next act, I will do a tf_idf on the entire corpus and then sort by score, descending

mb_tf_idf <- mb_texts %>%
  count(doc_id, word, sort = TRUE)  %>%  
  bind_tf_idf(word, doc_id, n) 

# lets just count basic words (with custom stops)

mb_texts_count <- mb_texts %>%
    count(word, sort = TRUE)

# lets not at least try to count by document to give us some fucking history, please. 
