...
sotu.speech.text <- tidy_sotu %>%
  count(president, doc_id, sotu_type)  %>% 
  group_by(sotu_type) %>% 
  summarize(avg_words = mean(n)) %>% 
  arrange(desc(avg_words))
## not sure if this is what you are looking for, but we now have an average of length based on written vs spoken STOU. 
```
